...
Arel is a library that active record uses to allow us developers to write seemingly simple lines of code that are then converted into some database query language such as SQL. If we have a class User that has_many of a class Cart, then Active Record Queries can turn this single line of code:

{% highlight ruby %}
2.2.0 :057 > User.first.carts.create
  User Load (2.5ms)  SELECT  "users".* FROM "users"  ORDER BY "users"."id" ASC LIMIT 1
   (5.9ms)  begin transaction
  SQL (2.7ms)  INSERT INTO "carts" ("user_id", "created_at", "updated_at") VALUES (?, ?, ?)  [["user_id", 1101], ["created_at", "2016-03-03 21:58:01.030233"], ["updated_at", "2016-03-03 21:58:01.030233"]]
   (1.2ms)  commit transaction
 => #<Cart id: 2001, user_id: 1101, purchase_date: nil, created_at: "2016-03-03 21:58:01", updated_at: "2016-03-03 21:58:01">
{% endhighlight %}
...
Active Record and Arel then build the next part of our line of code, `.carts.create`. It writes a query to add a Cart (`INSERT INTO "carts"`), with our user ID, and it even adds the created_at and updated_at datetimes for use (`("user_id", "created_at", "updated_at") VALUES (?, ?, ?)  [["user_id", 1],`
 `["created_at", "2016-03-03 19:49:04.369863"], ["updated_at", "2016-03-03`
  `19:49:04.369863"]]`).
...
By default, Active Record Queries only return and array (or more accurately an ActiveRecord::Associations::CollectionProxy) or an instance of the object you called the search on. Here is a quick example to demonstrate this point.
...
Ok, a bunch of nil User objects... What did my query even return? It's clear that whatever I wanted, Active Record is only going to return to me User objects here.
...
To solve this problem, I sometimes like to look at my SQL queries in a GUI (Graphical User Interface). Currently I use [SQLiteBrowser] (http://sqlitebrowser.org/). Using this program I can run my SQL queries to see what I'm really getting back.

Looking up to the previous example where I wanted a list of product names, you can see in the code snippet that the console actually outputs the raw SQL query. If I take that query and put it into SQLiteBrowser, I can now see the table that the database is returning:
...
This very clearly shows me that my SQL query created by Active Record and Arel returns the above table. The above table was then converted into User objects. Because there is no way to add `price:` to a User, they all came back nil and we were none the wiser to the table that our SQL returned. SQLiteBrowser can be used anytime you are confused about what data you are actually calling back from the database when you are writing complex queries.
...
One of the Learn students, [Brett Heenan](https://github.com/ryuichi7) brought my attention to the `#pluck()` method. This will pull out specified attributes from an Active Record query. This method would be perfect for the previous example where I was trying to get all of the products owned by a user. Let's give it a try:
...
My first step here is to get users and products on the same table together. I know I'm looking to get back Users as my return value, so I'll be calling this query on users. I also know that all I need from my products table will be price (I don't care about the name of the product). This is a complex query, so let's just try and get this to show up in SQLiteBrowser first. (I'm using my terminal to convert my Active Record query into SQL). I decided to order my results by user's name as well so that I'll be able to see all their rows together.
...
This is a good starting point. Next we know that as we are interested in total per user, we know we will have to group our data by user. So lets add a `.group()` to our query.
...
Woo! We now have the total each customer spent. Now we just have to say that we only want the ones that spend more than 5600. Lets also order it by their total_spent in s descending order.
...
{% highlight ruby %}
2.2.0 :081 > User.select("users.*, sum(products.price) as 'users_total'").joins(carts: { line_items: :product}).group("users.id").order("users_total desc").
...
<figure align='center'>
  ...
  <figcap>Filter out only high rollers and sort by total_spent</figcap>
</figure>
...
Some important tips to remember when crafting complex Active Record queries:
 ...
 - If you are more comfortable with raw SQL, it's OK to start by writting that out first and getting it working and then trying to work your way back to the Active Record syntax.
